...
async function fetchDataForYear(url: string, year: string, format: string) {
	...
	if (contribText) {
		...
		contribCount = Number.parseInt(contribCount.replace(/,/g, ""), 10);
	}
...
export async function fetchDataForAllYears(username, format) {
	const fYears = await fetchYears(username);
	console.log("ffff", fYears);
	const years = fYears.filter((data) => data.text === "2024");
	return Promise.all(
